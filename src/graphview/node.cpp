...
QRectF Node::boundingRect() const
{
	qreal adjust = 20;
	//int m_diameter = scene->m_diameter();
	return QRectF( -m_diameter/2 - adjust, -m_diameter/2 - adjust, m_diameter+ adjust*2, m_diameter + adjust*2);
}
...
void Node::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *)
{
	...
	if(childrenList.contains(this))
	{
		...
		painter->drawEllipse(-m_diameter*2/3,-m_diameter*2/3,m_diameter/2,m_diameter/2);
		painter->setBrush(Qt::black);
		painter->drawPolygon(QPolygonF() << QPointF(-17*m_diameter/30,-m_diameter/3) << QPointF(-7*m_diameter/15,-m_diameter/6)<< QPointF(-19*m_diameter/30,-m_diameter/6));
	}
	...
	if(shape_type == Rectangle)
		...
		painter->drawPolygon(QPolygonF() << QPointF(-m_diameter/2,0) <<QPointF(0,-m_diameter/2) <<
							 QPointF(m_diameter/2,0) << QPointF(0, m_diameter/2));
