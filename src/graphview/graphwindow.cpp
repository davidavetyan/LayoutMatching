...
void GraphWindow::saveTextGraph()
{
    ...
    QString FileName =
        QFileDialog::getSaveFileName(this, tr("Save file"), readPathSettings(), tr("text(*.txt)"));
    if (FileName.isEmpty())
        return;
                                        ...
                                        std::vector<std::pair<int, int>>* pSpecialEdges)
{
    ...
    for (int i = 0; i < adjMatrix.size(); i++)
    {
        ...
        for (int j = 0; j < adjMatrix[i].size(); j++)
        {
            ...
            if (edge_weight)
            {
                ...
                if (pSpecialEdges)
                {
                    auto it = std::find((*pSpecialEdges).begin(), (*pSpecialEdges).end(),
    ...
    ui->main_graph->getMscene()->createGraphWithText(nodes, child_of_nodes, &edge_weights,
                                                     &special_edges);
}
...
void GraphWindow::setDelay(int a)
{
    msec_delay = a;
}
...
void GraphWindow::wide_search()
{
    ...
    QString text = QInputDialog::getText(getMscene()->getMGraph(), "Search By Vertex",
                                         "Vertex name:", QLineEdit::Normal, "for searching", &ok);
    if (!ok)
        return;
    text = text.trimmed();
    ...
    while (!queue.isEmpty() && processing)
    {
        ...
        if (n->get_val() == text)
        {
            QMessageBox::information(this, " ", "Vertex " + text + " not found",
                                     QDialogButtonBox::Ok);
            slider->close();
            clearFormat();
            n->setColor(Qt::blue);
...
void GraphWindow::closeEvent(QCloseEvent* event)
{
    if (processing)
    {
        int b = QMessageBox::question(this, "?", "Algorithm not over yet. Force stop?",
...
void GraphWindow::hamilton_alg()
{
    ...
    if (getMscene()->isOriented())
    {
        QMessageBox::warning(this, "!", "Graph must be non-oriented (can be changed in settings)",
                             QMessageBox::Ok);
